...
function TaskTextInEdit(id) {
    const card = card.find(card => card.id === id);
...
async function editTask(id) {
    ...
    await includeAddTask();
    templateOkBtn(id); // optional: button-Funktion
 ...
 */
function priorityImgCheck(priority) {
    if (priority === 'Urgent') return '/static/img/priorityHighInactive.svg';
    if (priority === 'Medium') return '/static/img/priorityMediumInactive.svg';
    if (priority === 'Low') return '/static/img/priorityLowInactive.svg';
}
    ...
    return `
        <div class="board-card-small" draggable="true" onclick="bigCard(${card.id})" ondragstart="startDragging(event, ${card.id})">
            ...
            <div class="icons-area">
                ...
                <img src="/static/img/${card.priority.img}" alt="${card.priority.urgency}">
            </div>
        </div>
    `;
}
